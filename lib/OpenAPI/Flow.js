...
function formatResponses(output, error) {
  return {
    200: {
      description: '',
      content: {
        'application/json': {
          schema: typeSchema(output),
        },
      },
    },
    500: {
...
function pickParameter(input) {
  return (object) => lodash.mapValues(input, (entry, key) => {
    ...
    return value === undefined ? entry.default : TYPE_TABLE[schema.type](value);
  });
}
...
// ----------------------------------------------------------------------------
class Flow {
  constructor({ input, output, error, ...rest }) {
    this.object = {
      ...
      responses: formatResponses(output, error),
      ...rest,
    };

    this.input = pickParameter(input);
    this.output = pickBody(output);
    this.error = pickBody(error);
  }
